...
def lcv_order(board, r, c, candidates):
    """
    Return candidates sorted by least constraining value.
    High score = less constraining = try first
    """
    ...
    for v in candidates:
        ...
        for column in range(9):
            if board[r][column] == 0 and column != c and is_valid(board, r, column, v):
                score += 1

        for row in range(9):
            if board[row][c] == 0 and row != r and is_valid(board, row, c, v):
                score += 1
        ...
        for row in range(gb_v, gb_v + 3):
            for column in range(gb_h, gb_h + 3):
                if (row != r or column != c)  and board[row][column] == 0 and is_valid(board, row, column, v):
                    score += 1

        score_list.append((score, v))

    score_list.sort(reverse=True, key=lambda x: x[0])  # it will be sorted based on the number of score in descending order

    values = []
    for (score, v) in score_list:
        values.append(v)
    return values
...
def sudoku_solve(board):
    ...
    ordered = lcv_order(board, r, c, candidates)

    for v in ordered:  # the possible number can put into the empty box
