...
def getPredection(boxes, model):
    ...
    for image in boxes:
        ...
        # --- HERE: replace old calls ---
        probs = model.predict(img, verbose=0)          # shape: (1, C) or (1, 1)
        if probs.shape[1] == 1:
            # binary classifier (e.g., digit / no-digit)
            classIndex = (probs > 0.5).astype(int).ravel()   # [0] or [1]
            prob = float(probs[0, 0] if classIndex[0] == 1 else 1.0 - probs[0, 0])
        else:
            classIndex = np.argmax(probs, axis=1)            # [0..C-1]
            prob = float(np.max(probs))

        # 저장 (임계값은 상황에 맞게 0.7~0.9 조정)
        if prob > 0.8:
