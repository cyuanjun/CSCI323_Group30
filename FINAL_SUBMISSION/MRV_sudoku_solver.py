...
def find_mrv(board):
    ...
    * Among all empty cells, choose the one with the fewest valid candidates
    ...
    for r in range(9):
        for c in range(9):
            if board[r][c] == 0:
                candidates = get_candidates(board, r, c)  # candidates : the list of possible numbers [1-9]

                # update when a smaller candidate list is found
                if len(candidates) < min_len:
                    min_len = len(candidates)
                    best_cell = (r, c)  # the fewest number of possible nums's (r, c)
                    best_candidates = candidates  # the fewest number of possible nums's list

        # Early exit if a cell with only 1 candidate is found
        if min_len == 1:
            break

    if best_cell is None:  # When there's no more empty cell
        return None, None, None

    r,c = best_cell
    return r, c, best_candidates  # MRV cell and its candidate list


def sudoku_solve(board):
    stats["calls"] += 1  # added to evaluate recursive call

    r, c, candidates = find_mrv(board)  # the empty cell that has the lowest number of possible numbers
